...
import co.firstorderlabs.common.protos.{Observation, ObservationRequest, Reward, RewardRequest, RewardStrategy}
...
object Featurizer extends FeaturizerService with FeaturizerBase {
  def getObservation(
    ...
    val observation = Observation(construct(observationRequest), reward)

    Future.successful(observation)
  }

  def construct(observationRequest: ObservationRequest): List[Double] = {
    AccountFeaturizer.construct(observationRequest) ++ OrderBookFeaturizer
      .construct(observationRequest) ++ TimeSeriesFeaturizer.construct(
      observationRequest
    )
  }
