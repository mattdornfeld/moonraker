...
object Account {
    ...
    def calcRequiredBuyHold(order: BuyOrderEvent): QuoteVolume = {
      order match {
        case order: BuyLimitOrder => {
          ...
          order.price * order.size * Left(
            feeFraction.add(new BigDecimal("1.0"))
          )
        }
  ...
  object Account
    ...
    def getReceivedCancellations: Iterable[Cancellation] = {
      for (cancellation <- placedCancellations(
             Exchange.simulationMetadata.get.currentTimeInterval
           ))
        yield cancellation
    }
    ...
    def processMatch(matchEvent: Match): Unit = {
      matchesInCurrentTimeInterval append matchEvent
      val orderId = matchEvent.getAccountOrder.get.orderId
      val order = placedOrders(orderId)
      placedOrders(orderId) = OrderUtils.addMatchToOrder(order, matchEvent)
    ...
    override def getOrders(request: Empty): Future[Orders] = {
      val orders = Orders(
        placedOrders
          .map(
            item => (item._1.orderId, OrderUtils.getOrderSealedValue(item._2))
          )
          .toMap[String, Order]
      )
  ...
  object OrderRejecter {
    ...
    private def violatesPostOnly(
      ...
      val orderRequest =
        Account.getOrderRequest[LimitOrderRequst](limitOrder.requestId)
