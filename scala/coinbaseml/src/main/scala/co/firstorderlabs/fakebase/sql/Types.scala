...
import co.firstorderlabs.fakebase.Snapshot
import co.firstorderlabs.fakebase.currency.Configs.ProductPrice
import co.firstorderlabs.fakebase.currency.Configs.ProductPrice.ProductVolume
import co.firstorderlabs.fakebase.currency.Price.BtcUsdPrice.QuoteVolume
import co.firstorderlabs.fakebase.protos.fakebase._
import co.firstorderlabs.fakebase.types.Types.{OrderId, OrderRequestId, ProductId, TimeInterval}
...
case class QueryResult(
  ...
  def chunkByTimeDelta(timeDelta: Duration): List[QueryResult] = {
    val groupedBuyLimitOrders = buyLimitOrders.groupBy(x => timeInterval.getSubInterval(x.time, timeDelta))
    val groupedBuyMarketOrders = buyMarketOrders.groupBy(x => timeInterval.getSubInterval(x.time, timeDelta))
    val groupedCancellations = cancellations.groupBy(x => timeInterval.getSubInterval(x.time, timeDelta))
    val groupedSellLimitOrders = sellLimitOrders.groupBy(x => timeInterval.getSubInterval(x.time, timeDelta))
    val groupedSellMarketOrders = sellMarketOrder.groupBy(x => timeInterval.getSubInterval(x.time, timeDelta))
