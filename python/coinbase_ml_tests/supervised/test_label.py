...
@pytest.fixture
def volatility(prices: pd.Series) -> pd.Series:
...
@pytest.fixture
def labels_and_barriers(prices: pd.Series) -> pd.DataFrame:
...
def test_calc_triple_barrier_labels_and_barriers_point_wise(
    labels_and_barriers: pd.DataFrame, returns: pd.Series, expected_label: float
) -> None:
...
def test_calc_triple_barrier_label_and_barriers(
    volatility: pd.Series, returns: pd.Series
) -> None:
