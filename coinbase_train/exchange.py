...
from fakebase.exchange import Account as BaseAccount
...
from coinbase_train.utils.common import stdev
...
class Account(BaseAccount):
    ...
    def get_account_state_as_array(self) -> Dict[str, np.ndarray]:
        ...
        normalized_account_funds = account_funds / np.array(
            [
...
class Exchange(BaseExchange):  # pylint: disable=W0223
    ...
    def __init__(
        ...
        super().__init__(
            end_dt, num_workers, start_dt, time_delta, add_account, results_queue_size
        )
    ...
    def _get_order_book_as_array(
        ...
        return (
            np.array(
                [
                    (
                        min_max_normalization(price_normalization, 0, float(price)),
    ...
    def get_exchange_state_as_arrays(self) -> Dict[str, np.ndarray]:
        ...
        cancellation_statistics = [
            operator(self.received_cancellations)
            for operator in self._cancellation_operators
        ]

        match_statistics = [
            operator(self.matches) for operator in self._match_operators
        ]

        order_statistics = [
            operator(self.received_orders) for operator in self._order_operators
        ]

        state = dict(
            buy_order_book=self._get_order_book_as_array(
                order_side="buy",
            ...
            closing_price=self.matches[-1].price if self.matches else None,
            match_statistics=match_statistics,
            order_statistics=order_statistics,
            sell_order_book=self._get_order_book_as_array(
                order_side="sell",
                price_aggregation=c.ORDER_BOOK_BIN_SIZE,
